...
async function findEntryFirstPage(workspaceSlug: string, entryId: string) {
...
export async function generateMetadata({
  ...
  try {
    ...
    const description = entry.body
      ...
      .trim()
      .slice(0, 155)
    const url = absoluteUrl(`/${workspaceSlug}/changelog/${entryId}`)
    return {
      title,
      description,
...
export default async function PublicChangelogEntryPage({
  ...
  try {
    const [changelogMeta, found] = await Promise.all([
      fetchPublicChangelogSSR(workspaceSlug),
      findEntryFirstPage(workspaceSlug, entryId),
    ])
